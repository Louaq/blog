...
var rl = (e2) => {
  ...
  return e2.replace(il, (t2, l, a) => {
    if (a) return `<span style="color: slategray">${a}</span>`;
    if (l === "<") return "&lt;";
    let i;
...
var Ml = async () => {
  ...
  if (!e2 || e2.platform !== "Windows") return n2;
...
var sa = defineComponent({ __name: "ImageWall", props: { items: { default: () => [] }, columnWidth: { default: 300 }, gap: { default: 0 } }, emits: ["insert"], setup(e2, { expose: n2 }) {
  ...
  const a = ref(null), i = ref({}), h2 = ref([]), c2 = () => {
    ...
    if (h2.value.length === c2() && !g) return;
    h2.value = C2(c2());
...
![${m2.name}](${k})`);
    ...
    if ((m2 = c2.value) != null && m2.token && !t2.edit) T.nick = c2.value.display_name, T.mail = c2.value.email, T.link = c2.value.url;
    else {
      if (ge === "force") return;
      if (He.indexOf("nick") > -1 && !T.nick) return (s2 = C2.value.nick) == null || s2.focus(), alert(d2.value.nickError);
      if (He.indexOf("mail") > -1 && !T.mail || T.mail && !wl(T.mail)) return (k = C2.value.mail) == null || k.focus(), alert(d2.value.mailError);
    ...
    if (!te.value) return alert(d2.value.wordHint.replace("$0", Ve[0].toString()).replace("$1", Ve[1].toString()).replace("$2", z.value.toString()));
    ...
    try {
      ...
      if (Y.value = false, $e.errmsg) return alert($e.errmsg);
...
var Tn = defineComponent({ __name: "WalineComment", props: ["serverURL", "path", "meta", "requiredMeta", "dark", "commentSorting", "lang", "locale", "pageSize", "wordLimit", "emoji", "login", "highlighter", "texRenderer", "imageUploader", "search", "copyright", "recaptchaV3Key", "turnstileKey", "reaction"], setup(e2) {
  ...
  const f2 = (d2) => {
    ...
    if (M.value) M.value.comment = d2.comment, M.value.orig = d2.orig;
    else if ("rid" in d2) {
...
var Nn = ({ el: e2 = "#waline", path: n2 = window.location.pathname, comment: t2 = false, pageview: l = false, ...a }) => {
  ...
  if (e2 && !i) throw new Error("Option 'el' do not match any domElement!");
  if (!a.serverURL) throw new Error("Option 'serverURL' is missing!");
...
import "/workspaces/blog/node_modules/.pnpm/@waline+client@3.2.0/node_modules/@waline/client/dist/waline.css";
...
import Live2d from "/workspaces/blog/node_modules/.pnpm/vitepress-theme-website@1.0.8/node_modules/vitepress-theme-website/dist/live2d/components/Live2dComponent.vue";
Live2d.init = (props) => {
  const container = document.createElement("div");
